...
interface NavigationItem {
  ...
  const solutions: SolutionItem[] = [
    ...
    { 
      name: 'Portfolio URL', 
      description: 'Showcase your work and projects with a professional portfolio.', 
      href: '#', 
  ...
  const resources: SolutionItem[] = [
    { name: 'Founding Stories', description: ' Explore the origins and journeys of Nestsite, from humble beginnings to industry leaders', href: '#', icon: ChartPieIcon },];
...
const Header = ({ mobileMenuOpen, setMobileMenuOpen }: HeaderProps) => {
  ...
  return (
    <div >
      ...
      <header className={`fixed inset-x-0  z-50 ${pathname == '/' && !isScrolled ?'bg-transparent':'bg-black bg-opacity-80'} ${isScrolled ? 'top-0 bg-black bg-opacity-80 rounded-full my-4 mx-2 lg:mx-4' : 'bg-hero-image bg-cover bg-center'} `}>
        ...
        <div className="flex lg:flex-1">
            ...
            <FlyoutMenu solutions={solutions} callsToAction={callsToAction} mobileMenu={mobileMenuOpen} />
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
               ...
               <ResourceMenu resources={resources} RcallsToAction={RcallsToAction} mobileMenu={mobileMenuOpen} />
            </div>
            <div className="hidden lg:flex lg:flex-1 lg:justify-end">
              ...
              <Link href="#" className="text-white text-lg font-semibold leading-6 py-2.5">
                Login
              </Link>
            </div>
          </nav>
...
function FlyoutMenu({ solutions, callsToAction, mobileMenu }: { solutions: SolutionItem[]; callsToAction: CtaItem[]; mobileMenu: boolean }) {
    ...
    return (
      <Popover className="relative">
        <Popover.Button className="inline-flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900">
          <span className="text-white text-lg font-semibold leading-6">Solutions</span>
  ...
  function ResourceMenu({ resources, RcallsToAction, mobileMenu }: { resources: SolutionItem[]; RcallsToAction: CtaItem[]; mobileMenu: boolean }) {
    const dropdownClasses = mobileMenu
      ? 'mt-2 grid grid-cols-1 gap-y-1'
    ...
    return (
      <Popover className="relative">
        <Popover.Button className="inline-flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900">
          <span className="text-white text-lg font-semibold leading-6">Resources</span>
          <ChevronDownIcon className="h-5 w-5 text-white" aria-hidden="true" />
        </Popover.Button>
